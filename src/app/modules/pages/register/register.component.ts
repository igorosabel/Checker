...
import { LoginResult, RegisterData, RegisterValidation } from '@interfaces/user.interfaces';
...
export default class RegisterComponent {
  ...
  checkForm(): void {
    ...
    if (this.registerData.email !== null && !validateEmail(this.registerData.email)) {
    ...
    if (this.checkValidations()) {
      ...
      this.as.register(this.registerData).subscribe((result: LoginResult): void => {
        ...
        if (result.status === 'error-email') {
          this.ds.alert({
            title: 'Error',
            content: 'El email introducido ya está en uso. ¿Has olvidado tu usuario o contraseña?',
            ok: 'Continuar',
          });
        }
